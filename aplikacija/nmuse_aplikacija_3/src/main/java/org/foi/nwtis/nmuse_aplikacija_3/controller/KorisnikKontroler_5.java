...
@Path("administracijaPodrucja")
@Controller
public class KorisnikKontroler_5 {
    ...
    public String prikaziPodrucja() {
        ...
        List<String> svaPodrucja = new ArrayList<>();
        svaPodrucja.addAll(Arrays.asList("administracija", "administracijaAerodroma",
                "pregledKorisnik", "pregledJMS", "pregledDnevnik", 
                "pregledAktivnihKorisnika", "pregledAerodroma"));

        model.put("svaPodrucja", svaPodrucja);
        model.put("korisnikTrazi", korisnikTrazi);
        return "svaPodrucja.jsp";
    }
    ...
    private synchronized String izvrsiKomandu(String komanda, int port, String adresa) {
        try (Socket uticnica = new Socket(adresa, port);
            ...
            Logger.getLogger(KorisnikKontroler_5.class.getName()).log(Level.SEVERE, null, ex);
